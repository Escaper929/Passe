 ...
 * 物理幅面（v1.1）。
 *
 * 这一档的语义和像素档是**反**的：用户给的是"我要 A4、300DPI"，
 * 我们要反推出**需要多少像素**。具体数字由 `printSize.test.ts` 逐个钉死，
 * 这里只验证调校台把这条链路接对了：选纸才出现 DPI、改 DPI 反推值跟着动、
 * 源图不够时如实报数、文件名用的是反推出来的长边。
 */
describe('调校台 · 物理幅面（按目标 DPI 反推）', () => {
  /** ChoiceGrid 把说明挂在 title 上，按 title 找比按文字包含匹配稳。 */
  function buttonByTitle(title: string): HTMLButtonElement {
    ...
    if (!match) throw new Error(`找不到按钮：title=${title}`);
    return match;
  }

  const A4 = '297 × 210mm，小幅面成品（按目标 DPI 反推）';
  const A3 = '420 × 297mm，展览常用幅面（按目标 DPI 反推）';
  const DPI_300 = '艺术微喷 / 展览级';
  const DPI_150 = '校样 / 小样';

  /**
   * 读物理换算那两行。
   *
   * 第二行只在**没达到**目标 DPI 时才出现（`describeDpi` 的约定），
   * 所以 `actualDpi === null` 本身就是"达标"的信号 —— 这条约定值得锁住，
   * 否则将来有人给达标的情况也补一句"实际 300DPI"，面板会平白多一行噪音。
   */
  function printLine(): { requiredLongSide: number; actualDpi: number | null } {
    const need = /需源图长边 ≥ (\d+)px/.exec(text());
    if (!need) throw new Error('读不到反推的源图长边需求');
    const actual = /实际 (\d+)DPI/.exec(text());
    return { requiredLongSide: Number(need[1]), actualDpi: actual ? Number(actual[1]) : null };
  }

  it('像素档不谈 DPI，选了纸才出现 DPI 档与物理换算', async () => {
    await mount(<Harness onQueue={(q) => (latestQueue = q)} />);
    await seedImage(latestQueue!);

    // 默认是 8K 像素档：没有纸，DPI 无从谈起
    expect(text()).not.toContain('印在 ');
    expect(container?.querySelector(`button[title="${DPI_300}"]`)).toBeNull();

    await act(async () => {
      buttonByTitle(A4).click();
    });

    expect(text()).toContain('印在 A4（297 × 210mm）');
    expect(text()).toContain('：成品 ');
    expect(printLine().requiredLongSide).toBeGreaterThan(0);
    // 默认落在艺术微喷那一档，并且是选中态
    expect(buttonByTitle(DPI_300).className).toContain('border-white');
  });

  it('目标 DPI 越高、纸越大，反推需要的源图长边越大', async () => {
    dims = [6000, 4000];
    await mount(<Harness onQueue={(q) => (latestQueue = q)} />);
    await seedImage(latestQueue!);
    ...
    const a4at300 = printLine().requiredLongSide;

    await act(async () => {
      buttonByTitle(DPI_150).click();
    });
    const a4at150 = printLine().requiredLongSide;
    // 一半的 DPI，需要的像素也该少一半左右（不作精确断言，那是单元测试的事）
    expect(a4at150).toBeLessThan(a4at300);

    // 换大纸时 DPI 选择要保留，否则用户每换一次纸都得重选
    await act(async () => {
      buttonByTitle(A3).click();
    });
    const a3at150 = printLine().requiredLongSide;
    expect(a3at150).toBeGreaterThan(a4at150);
  });

  it('源图够用时按反推值输出，并且真的达到目标 DPI', async () => {
    // 3000 × 2000 对 A4@300 来说刚好够（反推约 2830），所以输出会被压一点点
    dims = [3000, 2000];
    await mount(<Harness onQueue={(q) => (latestQueue = q)} />);
    await seedImage(latestQueue!);
    ...
    const required = printLine().requiredLongSide;
    expect(required).toBeLessThan(3000);
    expect(planRow('输出').split(' × ')[0]).toBe(String(required));
    // 达标时不出现"不放大"那句降级说明
    expect(printLine().actualDpi).toBeNull();
  });

  it('源图不够时如实报实际 DPI，且绝不放大', async () => {
    dims = [1600, 1200];
    await mount(<Harness onQueue={(q) => (latestQueue = q)} />);
    await seedImage(latestQueue!);
    ...
    // 只降不升：输出就是源图本身，不会为了凑 DPI 凭空插值
    expect(planRow('输出')).toBe('1600 × 1200');

    const info = printLine();
    expect(info.requiredLongSide).toBeGreaterThan(1600);
    expect(text()).toContain('不放大');
    expect(info.actualDpi).not.toBeNull();
    expect(info.actualDpi!).toBeLessThan(300);
  });

  it('选了纸之后，面板上的文件名与实际下载的一致', async () => {
    dims = [3000, 2000];
    await mount(<Harness onQueue={(q) => (latestQueue = q)} />);
    await seedImage(latestQueue!);
    ...
    const shown = shownFilename();
    const longSide = Number(/_(?<side>\d+)px\.jpg$/.exec(shown)?.groups?.side);
    // 文件名里的长边必须是纸反推出来的那一个，而不是源图或某个像素档
    expect(longSide).toBe(printLine().requiredLongSide);
